...
author: "Alex Edwards | Omar Abdul-Rahman"
format: html
editor: visual
---
...
You may use any of the data in the 'data' folder listed in the \[Background\] section of this document or that has been created in the code above to answer these questions.

### Directions

You do not need to answer any questions with anything besides code and a map unless explicitly asked to do so.
...
2.  Create a map that shows the neighborhoods of Soho as hollow with no fill color and with **all** cases shown as points. Use the blank chunk below for your code.
...
index <- st_read("data/Cholera_Deaths_period_0_MOR.shp") 
cld1 <- st_read("data/Cholera_Deaths_period_1_JC.shp") 
cld2 <- st_read("data/Cholera_Deaths_period_2_ECB.shp") 
cld3 <- st_read("data/Cholera_Deaths_period_3_LW.shp") 
cld4 <- st_read("data/Cholera_Deaths_period_4.shp") 

cases_sf <- rbind(index, cld1, cld2, cld3, cld4)
...
3.  Now, reusing code from question one, set the color of each point to represent which time period that case occurred in. Use the blank chunk below for your code.
...
tm_shape(neighborhood_w_census)+
  tm_polygons(fill=)+
  tm_shape(index)+
   tm_dots(size = 0.25, fill = "purple", col = "purple")+
  tm_shape(cld1)+
   tm_dots(size = 0.25, fill = "red", col = "red")+
   tm_shape(cld2)+
   tm_dots(size = 0.25, fill = "blue", col = "blue")+
   tm_shape(cld3)+
   tm_dots(size = 0.25, fill = "orange", col = "orange")+
   tm_shape(cld4)+
   tm_dots(size = 0.25, fill = "brown", col = "brown")
    ```

4.  Create two maps below, one that shows the Soho neighborhoods with the cases from time period two and an additional map that shows the cases from time period three. Use the blank chunks provided to write your code.
...
5.  Look at the two maps you created for question three, did the cholera epidemic begin to abate between these two time points? Justify your answer. Answer using the bullet below

    Yes the cholera epidemic appears to begin to abate between these two time points 
    - First map, there is a much higher density and wider spatial distribution of cases, which could indicate a more widespread transmission     
    - Second map, the number of cases is lower and more clustered which can suggest a reduction in  the intensity and spread of the outbreak 
    

6.  Create a map that shows where the water pumps are located in relation to the Soho neighborhoods, set the color of the pumps to green and the size of the points to 0.4mm. Use the blank chunk below for your code.

    ```{r}
tm_shape(neighborhood_w_census)+
  tm_polygons(fill=)+
    tm_shape(pumps_sf)+
  tm_dots(size = 0.4, fill = "green")
    ```

7.  The pump with the ID number 7 is the Broad Street Pump that John Snow suspected of being the infected pump. For this map, reuse your code from question five and add an additional layer that highlights pump seven with the color red and a size of 0.55mm. See [Selection](#selection) for an example that would be helpful. Use the blank chunk below for your code.
...
8.  Use any of the data you like in this project and create a map of your choosing. Using the bullet point below to tell me what question the map answers and the blank chunk below for your code.

    -   This map will compare the density of Period 1 of the cholera outbreak (Gold) to Period 4 (Navy Blue) and it will be show the changes of concentations in contrast to the water pumps (Red) spread throughout the city.
    
    - The question I plan to answer is, which water pumps had the highest numbers of cholera cases between 2 periods of the outbreak? In addition, as the outbreak progressed did the number of cases wane between the two periods displayed? If so, did did the locations of the outbreak hotspots change and what pumps were in close proximity to the later outbreaks? 
    - Compared to outbreak 1 outbreak 4 had decreased dramatically. In addition, during the first outbreak, despite the concentration of the cases being highest in the center of Soho the amount of cases spread partway through the east, west, and stretched to the south end of Soho. During the 4th outbreak the cases began clustering towards the center of Soho and was basically surrounded pump 7, that pump being the only pump during this time period to still have high concentrations of cholera patients save some outliers throughout the city.  
...
tm_shape(neighborhood_w_census)+
  tm_polygons(fill=)+
   tm_shape(cld1)+
   tm_dots(size = 0.25, fill = "darkgoldenrod1", col = "darkgoldenrod1")+
   tm_shape(cld4)+
   tm_dots(size = 0.25, fill = "navy", col = "navy")+
    tm_shape(pumps_sf)+
  tm_dots(size = 0.25, fill = "red")
    ```
